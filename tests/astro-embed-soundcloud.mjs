...
test('matcher returns canonical URL for a track', () => {
	assert.is(
		matcher('https://soundcloud.com/forss/flickermood'),
...
test('matcher strips www.', () => {
	assert.is(
		matcher('https://www.soundcloud.com/forss/flickermood'),
		'https://soundcloud.com/forss/flickermood'
	);
});
...
test('matcher works without protocol', () => {
	assert.is(
		matcher('soundcloud.com/forss/flickermood'),
		'https://soundcloud.com/forss/flickermood'
	);
});
...
test('it renders a lite-soundcloud element for a valid URL', async () => {
	...
	assert.ok(embed || fallback, 'should render either lite-soundcloud or a fallback link');
});
...
test('it sets data-url on the lite-soundcloud element', async () => {
	...
	const embed = window.document.querySelector('lite-soundcloud');
	if (embed) {
		assert.is(embed.getAttribute('data-url'), 'https://soundcloud.com/forss/flickermood');
	}
});
